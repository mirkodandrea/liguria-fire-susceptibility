...
clusterDataset<- function(dataset){
  # veg_type one hot encoding
  datasetTmp <- dataset 
  datasetTmp <- subset(datasetTmp,select=-fire)  # without fire column
  # datasetTmp@data[["fire"]] <- strtoi(datasetTmp@data[["fire"]])
  ...
  dataset <- dataset %>% 
    subset(.@data$cluster ==1)# selecting one class type 
...
build_dataset <- function(points_df, fires_df, season, year_from, year_to) {
  ...
  sel_fires_df <- season_fires_df %>% 
                  ...
                  subset(.$year < year_to)
  # this is the case when the user specifies a specific vector of years  for test.
  }else{ 
    sel_fires_df <- season_fires_df %>% 
                    subset(.$year >= year_from) %>%
...
# create the spatial grid over the region for doing the cross validation
...
# create a dataset filtered with some indeces 
...
# function for a single fold selection 
train_single_fold <- function(dataset, test_ratio=0.2, columns, model,algo, ...) {
  ...
  if(algo == "mlp"){ 
    ...
    # the max-imum output difference.
...
# training over the differnet selected folds 
train_on_folds <- function(dataset, folds, columns, model, algo, ...) {
  ...
  if(algo == "mlp"){ 
    ...
    print("inside svm model generation and pred_TS computing")#ksvm
    RF <- model(fire~., TR, model="svm",task="prob")# rminer 1.3.1 calls it svm
    pred_TS <- predict(object = RF,
                       newdata = TS[, names(TS) != "fire"])
                       #,type = "prob")  RMINER 1.3.1 
  }else if(algo ==  "mlp_rminer"){ 
...
# function for creating the final raster (susceptibility map) 
...
# function for creating the dataset and doing the training
...
# funciton for running the model
